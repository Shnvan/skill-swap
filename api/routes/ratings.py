...
from api.db import rating_table, task_table
...
def user_participated_in_task(user_id: str, task_id: str) -> bool:
    try:
        ...
        if not task:
            return False
        # Check if user is creator, accepter, or completer of the task
        return user_id in (
            task.get("creator_id"),
            task.get("accepted_by"),
...
def create_rating(rating: RatingCreate, from_user: str = Depends(get_current_user)):
    if not user_participated_in_task(from_user, rating.task_id):
        raise HTTPException(
            status_code=403,
            detail="You can only rate users for tasks you participated in.",
        )

    # Query existing rating by this user for this to_user and task
    ...
    if existing_rating.get("Items"):
        raise HTTPException(
            status_code=400, detail="You have already rated this user for this task."
        )
    ...
    item["from_user"] = from_user  # override client value
...
def get_ratings_for_user(user_id: str):
    try:
        response = rating_table.query(
            ...
            KeyConditionExpression=Key("to_user").eq(user_id),
            FilterExpression=Attr("is_flagged").eq(False),
        )
...
def flag_rating(
    ...
    flagged_by: str = Depends(get_current_user),
):
    try:
